...
!(function(t, i) {
            ...
            t = l.Marker.extend({
                ...
                _getIconSVG(t, i) {
                    ...
                    return {
                        ...
                        svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${t}" height="${t}" version="1.1" viewBox="-${o} -${o} ${t} ${t}">` +
                            '<circle r="' +
                            s +
                            '" style="' +
                            i +
        ...
        var e = t.extend({
                    ...
                    _getIconSVG(t, i) {
                        ...
                        return {
                            ...
                            svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${o}" height="${s}" version="1.1" viewBox="-${
              o / 2
    ...
    e = l.Control.extend({
      options: {
        ...
        createButtonCallback(t, i) {
          const s = l.DomUtil.create(
            "a",
            "leaflet-bar-part leaflet-bar-part-single",
            t
          );
          (s.title = i.strings.title), (s.role = "button"), (s.href = "#");
          const o = l.DomUtil.create(i.iconElementTag, i.icon, s);
          if (void 0 !== i.strings.text) {
            const e = l.DomUtil.create(
              i.textElementTag,
              "leaflet-locate-text",
              s
            );
            (e.textContent = i.strings.text),
              s.classList.add("leaflet-locate-text-active"),
              (s.parentNode.style.display = "flex"),
              0 < i.icon.length && o.classList.add("leaflet-locate-icon");
          }
          return { link: s, icon: o };
        },
        onLocationError(t, i) {
          alert(t.message);
        },
        ...
        if (
          ...
          switch (
            (this._map.getBounds().contains(this._event.latlng) &&
              (t = i ? s.inView : s.inViewNotFollowing),
            s[t] && (t = s[t]),
            t)
          ) {
            case "setView":
            ...
            case "stop":
              if ((this.stop(), this.options.returnToPrevBounds)) {
                const o = this.options.flyTo
                  ? this._map.flyToBounds
                  : this._map.fitBounds;
                o.bind(this._map)(this._prevBounds);
              }
          }
        } else
      ...
      _drawMarker() {
        ...
        this.options.drawCircle &&
          ((i = this._isFollowing()
            ...
            : this.options.circleStyle),
          this._circle
            ? this._circle.setLatLng(s).setRadius(t).setStyle(i)
            : (this._circle = l.circle(s, t, i).addTo(this._layer)));
        let o, e;
        (e = this.options.metric
          ? ((o = t.toFixed(0)), this.options.strings.metersUnit)
          : ((o = (3.2808399 * t).toFixed(0)), this.options.strings.feetUnit)),
          this.options.drawMarker &&
            ...
            this._marker
              ? (this._marker.setLatLng(s),
                this._marker.setStyle && this._marker.setStyle(i))
              : (this._marker = new this.options.markerClass(s, i).addTo(
                  this._layer
                ))),
          this._drawCompass();
  ...
  return (l.control.locate = (t) => new l.Control.Locate(t)), e;
}, window);
